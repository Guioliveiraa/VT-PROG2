...
float lerArquivo (char path [], int id);
...
main (){
	...
	while (1){
		...
		char novoPreco[10], novoSaldo[10];
		...
		switch (opcao){
			...
			case 3:
				...
				scanf ("%s", novoSaldo);
				editFile ("Clientes.txt", ";",idCliente,4,novoSaldo);
				break;
...
void fechaCaixa (vendas *v){
    ...
    for (i = 0; i < 100; i++){
		...
		novoSaldo = v[i].quantidadeProdutos * lerArquivo("Produtos.txt", v[i].codigoProduto);
		strcpy (saldoCopiado, (char)novoSaldo);
		editFile("Clientes.txt", ";", v[i].idCliente, 4, saldoCopiado);
	}
	fclose (arq);
	free (v);
}
...
float lerArquivo (char path [], int id){
	...
	while (!feof(arq)){
		fscanf(arq, "%d; %*[^;]; %f;" , &idProduto, &preco);
		//printf ("%f %d %d\n", preco, id, idProduto);
		if (idProduto == id){
			return preco;
			break;
