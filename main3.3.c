...
void alteraProduto(){
	...
	char produto[50];
	int ch;
	float precoProduto, preco;
	int idProduto, id, i=0;
	FILE*arq;
	...
	if(arq == NULL){
		...
		system("cls");
		ch=contaLinhaProduto();
		printf("Diga o ID do produto que deseja alterar:\n");
		scanf("%d", &idProduto);
	    ...
	    while(i<ch){
			fscanf(arq, "%d;%[^;];%f;", &id, produto, &preco);
			if(idProduto==id){
				arq = fopen("Produtos.txt","a");
				fprintf(arq,"%d;%s;%.2f;\n", idProduto, produto, precoProduto);
			}
			i++;
		}
	}
